  ...
  return (
    <Link
      ...
      className={cn(
        ...
        featured && "min-w-[252px] sm:min-w-[280px]",
      )}
    >
      ...
      <div className="p-4">
        <div className="flex items-start justify-between gap-3">
          <div className="min-w-0">
            <h3 className="truncate font-display text-xl font-semibold tracking-[-0.025em] text-[#063b43]">{item.name}</h3>
            <p className="mt-1 line-clamp-2 text-sm leading-5 text-[#527177]">{item.description}</p>
          </div>
          <ChevronRight className="mt-1 h-5 w-5 shrink-0 text-[#12909f] transition-transform group-hover:translate-x-0.5" />
        </div>
        <div className="mt-4 flex items-center justify-between gap-2 border-t border-[#e4efed] pt-3 text-xs font-semibold text-[#467278]">
