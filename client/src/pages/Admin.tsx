...
import { CheckCircle2, ChevronRight, CircleDollarSign, ImagePlus, LayoutDashboard, Loader2, MapPin, Pencil, Plus, Sparkles, Store, Tag, Tags, Trash2, WalletCards, X } from "lucide-react";
import { ChangeEvent, FormEvent, useEffect, useMemo, useState } from "react";
import { Link } from "wouter";
import { toast } from "sonner";

type AdminTab = "inicio" | "estabelecimentos" | "categorias" | "planos" | "mensalidades" | "destaques" | "pix";
...
function AdminContent() {
  ...
  const [paymentForm, setPaymentForm] = useState({ establishmentId: "", planId: "", amount: "", dueAt: toDateValue(new Date()), notes: "" });
  ...
  const createSubscription = trpc.admin.createSubscription.useMutation({ onSuccess: () => { refresh(); toast.success("Mensalidade registrada."); } });
  const updateSubscription = trpc.admin.updateSubscriptionStatus.useMutation({ onSuccess: () => { refresh(); toast.success("Status atualizado."); } });
  ...
  const submitPayment = (event: FormEvent) => { event.preventDefault(); if (!paymentForm.establishmentId || !paymentForm.planId || !paymentForm.dueAt) return toast.error("Preencha estabelecimento, plano e vencimento."); createSubscription.mutate({ establishmentId: Number(paymentForm.establishmentId), planId: Number(paymentForm.planId), amountCents: Math.round(Number(paymentForm.amount || 0) * 100), dueAt: new Date(`${paymentForm.dueAt}T12:00:00`), notes: paymentForm.notes || null, status: "pendente" }); };
