      ...
      <div className="container max-w-6xl py-7 sm:py-10">
        ...
        <div className="mt-6 grid overflow-hidden rounded-[2rem] bg-white shadow-[0_28px_60px_rgba(6,59,67,0.12)] lg:grid-cols-[1.15fr_0.85fr]">
          ...
          <section className="flex flex-col p-7 sm:p-10">
            <div className="flex flex-wrap items-center gap-2"><p className="text-xs font-extrabold uppercase tracking-[0.18em] text-[#d68d20]">{establishment.categoryName}</p>{establishment.isDemo && <span className="rounded-full bg-[#fff0ca] px-2.5 py-1 text-[0.6rem] font-extrabold uppercase tracking-[0.12em] text-[#7e5915]">Estabelecimento demo</span>}</div>
            ...
            <div className="mt-auto grid gap-3 pt-8">{establishment.isDemo ? <Button onClick={() => toast("Este é um estabelecimento demo; o contato real ainda não está ativo.")} className="h-12 w-full rounded-full bg-[#0e916e] text-sm font-extrabold text-white shadow-[0_8px_18px_rgba(14,145,110,0.25)] transition hover:bg-[#087454]"><Phone className="h-4 w-4" /> Chamar no Zap</Button> : <a href={whatsappUrl(establishment.whatsapp)} target="_blank" rel="noreferrer"><Button className="h-12 w-full rounded-full bg-[#0e916e] text-sm font-extrabold text-white shadow-[0_8px_18px_rgba(14,145,110,0.25)] transition hover:bg-[#087454]"><Phone className="h-4 w-4" /> Chamar no Zap</Button></a>}{!establishment.isDeliveryOnly && <a href={mapsUrl} target="_blank" rel="noreferrer"><Button variant="outline" className="h-12 w-full rounded-full border-[#0b7e8a]/25 bg-transparent text-sm font-extrabold text-[#0a6874] hover:bg-[#ecf8f6]"><Navigation className="h-4 w-4" /> Como Chegar</Button></a>}</div>
          </section>
        </div>
      </div>
    </main>
